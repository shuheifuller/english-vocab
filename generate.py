...
/* ── LOCK ─────────────────────────────────────── */
#lock{{position:fixed;inset:0;background:var(--bg);z-index:999;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:24px;padding:40px 24px}}
.lock-logo{{font-size:56px;line-height:1}}
.lock-title{{font-size:26px;font-weight:800;letter-spacing:-.5px}}
.lock-sub{{font-size:13px;color:var(--tx2);text-align:center;max-width:260px;line-height:1.6}}
.g-btn{{display:flex;align-items:center;gap:12px;background:#fff;color:#3c4043;border:1px solid #dadce0;border-radius:24px;padding:12px 24px;font-size:15px;font-weight:500;cursor:pointer;width:100%;max-width:280px;justify-content:center;transition:box-shadow .2s;font-family:inherit}}
.g-btn:hover{{box-shadow:0 2px 10px rgba(0,0,0,.15)}}
.lock-err{{color:var(--ac);font-size:12px;text-align:center;min-height:18px}}
...
<div id="lock">
  ...
  <div class="lock-sub">Shuheiの個人英語学習帳<br>Googleアカウントでログインしてください</div>
  ...
  <div class="ssec">
    ...
    <div class="guide-card">
      ...
      <div class="guide-item">
        <div class="guide-icon">📱</div>
        <div>
          <div class="guide-title">オフラインで追加</div>
          <div class="guide-body">「追加」タブで入力 → キューに保存。PCに戻ったら <span class="guide-code">queue.json をダウンロード</span> → <span class="guide-code">python3 sync.py</span> を実行。</div>
        </div>
      </div>
      <div class="guide-item">
        <div class="guide-icon">🃏</div>
...
function addW() {{
  ...
  const now = new Date();
  pq.push({{word: w, notes: document.getElementById('an').value.trim(),
